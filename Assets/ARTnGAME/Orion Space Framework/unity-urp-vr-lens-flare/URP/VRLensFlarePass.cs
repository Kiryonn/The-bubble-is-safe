    ...
    public class VRLensFlarePass : ScriptableRenderPass
    {
        ...
        private void CalculateRayVisibility(VRLensFlare lensFlare, bool isDirLight)
        {
            if (lensFlare.UseOcclusion)
            {
                ...
                Vector3 sun_pos = -lensFlare.transform.forward * 10000000;//
                var ray = new Ray(_camera.transform.position, sun_pos-_camera.transform.position);

                if (Physics.Raycast(ray, out var hit, Mathf.Infinity, _occlusionLayerMask))
