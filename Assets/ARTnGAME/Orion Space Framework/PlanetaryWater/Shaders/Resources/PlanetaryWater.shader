Shader "WaterTesselated"
{
    Properties
    {
		...
		//v0.1
		 _BumpMapA("Normal Map A", 2D) = "bump" {} 
		 _Bump2Scale ("Normal 2 scale", float) = 1
		  [HDR]
		 _SpecularColor ("_SpecularColor water color", Color) = (0.44, 0.95, 0.36, 1.0)
        ...
        [Header(Tesselation)]
        _TesselationPower("Tesselation power", Range(1, 96)) = 2
        _MinTesslationDistance("Min Tesselation distance", Range(10, 1000)) = 25
        _MaxTesslationDistance("Max Tesselation distance", Range(10, 500000)) = 100 //v0.1
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings TessellationVertex(TessControlPoint input)
            {
				...
				output.parameters = float4(length(wavePosition) - _Radius, positionOS);// 0, 0, 1); //v0.9b
			...
			// from GPU Pro 7 - remaps value from one range to other range //v0.9a
            ...
            float4 Fragment(Varyings input) : SV_Target
            {
				...
				//v0.1
				half3 normalTs = SampleNormal(normalUV.xy * 0.001*_Bump2Scale, TEXTURE2D_ARGS(_BumpMapA, sampler_BumpMapA))*0.5;
				//normalWS = normalWS / (length(viewDirWS)+0.1) 
				//+ normalize(normalTs.x * input.tangentWS + normalTs.y * input.bitangentWS + normalTs.z * input.normalWS) * length(viewDirWS);
				float dist = length(viewDirWS);
				half3 norm2 = normalize(normalTs.x * input.tangentWS + normalTs.y * input.bitangentWS + normalTs.z * input.normalWS) * length(viewDirWS);
				normalWS = clamp(normalWS * (10000 - dist)/20111,-1,1) + 1.1*clamp(norm2 * (10000 - dist)/21011,-1,1);
				...
				Light mainLight = GetMainLight();// shadowCoord, positionWS, float4(0, 0, 0, 0)); //v0.1
				...
				float4 coordinates = TransformWorldToShadowCoord(positionWS / (1 - 0.0));
				...
				//directLight = directLight * pow(attenuate, 1);
                ...
                {
					Light light = GetAdditionalLight(i, positionWS);// , float4(1, 1, 1, 1)); //v0.1
...
#ifdef _ADDITIONAL_LIGHTS
                for (int i1 = 0; i1 < additionalLightsCount; i1++)
                {
					Light light = GetAdditionalLight(i1, positionWS);// , float4(1, 1, 1, 1)); //v0.1
				...
				//v0.9
				sunSpecularMask *= (attenuate - 0.12);
				...
				//v0.9
				sunSpecularColor *= (attenuate-0.12);
				...
				finalColor += sunSpecularColor * 0.4*_SpecularColor.rgb + subsurfaceScatteringColor + additionalLightsColor;
				...
				//SELF SHADOW !!!	
				//if (specialFX.y != 0) {
					float checkVar = -0.25 * 1;
					float dotSun = dot(_MainLightPosition.xyz, normalize(input.parameters.yzw));// normalize(positionWS));// _SunDir);  //v0.9b
					if (dotSun < checkVar)
					{
						finalColor.rgb = 0;
					}
					...
					{
						finalColor.rgb *= remap(dotSun, checkVar, 1, 0, 2.4*(1 + 0));
					}
				//}

					//v0.9
					//finalColor *= attenuate;
