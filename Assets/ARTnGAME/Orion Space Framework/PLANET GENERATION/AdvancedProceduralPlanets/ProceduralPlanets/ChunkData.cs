...
namespace Artngame.Orion.PlanetGEN
{
    ...
    public class ChunkData : IDisposable
    {
        ...
        public IEnumerator StartGenerateCoroutine(bool fastMode)//v0.1
        {
            WantsRefresh = false;
        ...
        void GenerateHeightMap()
        {
            if (chunkConfig.rescaleToMinMax)
            {
                ...
                if (heightRough == null)
                {
                    heightRough = new RenderTexture(HeightMapResolution / 2, HeightMapResolution / 2, 0, RenderTextureFormat.RInt, RenderTextureReadWrite.Linear);
        ...
        void RequestMeshData()
        {
            ...
            if (!vertexCPUBufferCreated)
            {
                vertexCPUBuffer = new NativeArray<PerVertexData>(chunkConfig.NumberOfVerticesNeededTotal, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
                vertexCPUBufferCreated = true;
            }
