Shader "Unlit/AtmosSRP_FORWARD_URP_MULTIPLANET_A"
{
	...
	//half4 _CameraDepthTexture_ST;
			...
			*/
			...
			float4 frag(v2fATMOS i) : SV_Target
			{
				...
				int fragment_msA = 0;
				float fragment_depthA = tex2D(_CameraDepthCustom, i.uv - float2(0, fragment_size.y)).r;
				fragment_msA += fragment_depthA != 0;
				fragment_depthA = tex2D(_CameraDepthCustom, i.uv - float2(fragment_size.x, 0)).r;
				fragment_msA += fragment_depthA != 0;
				fragment_depthA = tex2D(_CameraDepthCustom, i.uv + float2(0, fragment_size.y)).r;
				fragment_msA += fragment_depthA != 0;
				fragment_depthA = tex2D(_CameraDepthCustom, i.uv + float2(fragment_size.x, 0)).r;
				fragment_msA += fragment_depthA != 0;
				fragment_depthA = tex2D(_CameraDepthCustom, i.uv).r;
				fragment_msA += fragment_depthA != 0;
				...
				/*	if (i1==0) {
					...
					if ( fragment_msA > 4 )//9 //4)//if (fragment_ms > 6)//4) //depthSample < 0.0015)//  
					{
						float fragment_distance = LinearEyeDepth(fragment_depthA);
						...
						if (distance_to_intersection < 0 && length(camera - earth_center) > 639100 * 1.2) //if (i1 == 0 && length(camera - earth_center) > 639100 * 1.4 && distance_to_intersection < 0) ///  
						{
							radiance = 0;
						}
					...
					//if (i1 == 0 && length(camera - earth_center) > 639100 * 1.4) //  
					//{
					//	radiance = 0;
					//}


					//if ( depthSample < 0.5 * 0.1 * skyControlsPlanetB.y && length(camera - earth_center) < 639100 * 2.4 && i1 == 0)//  
					//{
					//	radiance = 0;
					//}
