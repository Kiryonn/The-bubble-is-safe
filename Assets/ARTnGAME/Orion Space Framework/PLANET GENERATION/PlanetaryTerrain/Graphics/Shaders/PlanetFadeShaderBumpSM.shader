...
Shader "PlanetaryTerrain/PlanetFadeShaderBumpSM" {
	Properties {
	...
	SubShader {
		...
		//Tags { "SHADOWSUPPORT"="true" "RenderType"="Opaque" }
		...
		//};
		  ...
		  struct vertOut {
				...
				half fade : TEXCOORD2;
				float2 uv4_Tex2 : TEXCOORD3;				
            };

		 vertOut vert(appdata_full v) {
		//void vert (inout appdata_full v, out Input data) {
			vertOut o;
      		UNITY_INITIALIZE_OUTPUT(vertOut,o);///UNITY_INITIALIZE_OUTPUT(Input,data);
      		float pos = length(UnityObjectToViewPos(v.vertex).xyz);
      		float diff = _FadeRangeMax - _FadeRangeMin;
      		float invDiff = -1.0 / diff;
      		o.fade = clamp ((_FadeRangeMax - pos) * invDiff, 0.0, 1.0);
			o.pos = UnityObjectToClipPos (v.vertex);
            o.uv_Tex1 = v.texcoord;
			o.uv4_Tex2 = v.texcoord3;
			o.color = v.color;
            return o;
    	}
		...
		//void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			//o.Albedo 
			float4 albedo = IN.color.r * c1 + IN.color.g * c2 + IN.color.b * c3 + IN.color.a * c4 + IN.uv4_Tex2.x * c5 + IN.uv4_Tex2.y * c6;
