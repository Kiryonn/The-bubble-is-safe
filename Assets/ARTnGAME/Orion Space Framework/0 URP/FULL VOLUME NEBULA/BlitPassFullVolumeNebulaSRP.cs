...
namespace UnityEngine.Rendering.Universal
{
    ...
    /// </summary>
    internal class BlitPassFullVolumeNebulaSRP : UnityEngine.Rendering.Universal.ScriptableRenderPass
    {
#if UNITY_2023_3_OR_NEWER
        //GRAPH
        //v0.1
        ...
        public void OnCameraSetupA(CommandBuffer cmd, PassData renderingData)//(CommandBuffer cmd, ref UnityEngine.Rendering.Universal.RenderingData renderingData)
        {
            ...
            m_CameraColorTarget = renderingData.colorTargetHandleA; //v0.2
        }
#endif
        ...
        //v0.4  - Unity 2020.1
        public BlitFullVolumeNebulaSRP.BlitSettings settings;
        ...
        public override void OnCameraSetup(CommandBuffer cmd, ref UnityEngine.Rendering.Universal.RenderingData renderingData)
        {
            ...
            _handle = RTHandles.Alloc(settings.textureId, name: settings.textureId);      
            destination = (settings.destination == BlitFullVolumeNebulaSRP.Target.Color)
                ? renderer.cameraColorTargetHandle //UnityEngine.Rendering.Universal.RenderTargetHandle.CameraTarget //v0.1
                : _handle;          
        ...
        //v0.8a - PILLARS
        public int maxPillars = 32;
        public List<Transform> pillarTransforms = new List<Transform>();
        public float pillarLimiterA = 0;
        public float pillarFaderA = 0;
        public float pillarFaderB = 0;
        public List<int> pillarType = new List<int>();
        //v0.8 - PILLAR LIGHTS
       ...
       // public bool _fastest = false;
        ...
        //SUN SHAFTS         
        public BlitFullVolumeNebulaSRP.BlitSettings.SunShaftsResolution resolution = BlitFullVolumeNebulaSRP.BlitSettings.SunShaftsResolution.Normal;
        public BlitFullVolumeNebulaSRP.BlitSettings.ShaftsScreenBlendMode screenBlendMode = BlitFullVolumeNebulaSRP.BlitSettings.ShaftsScreenBlendMode.Screen;
        ...
        /// </summary>
        public BlitPassFullVolumeNebulaSRP(UnityEngine.Rendering.Universal.RenderPassEvent renderPassEvent, 
            Material blitMaterial, int blitShaderPassIndex, string tag, BlitFullVolumeNebulaSRP.BlitSettings settings)
        {
        ...
        connectSuntoFullVolumeNebulaURP connector;
        ...
        public override void Execute(ScriptableRenderContext context, ref UnityEngine.Rendering.Universal.RenderingData renderingData)
        {
            ...
            //v1.1.0
            if (Camera.main == null || (renderingData.cameraData.camera != Camera.main && !isForReflections))
            {
            ...
            connector = renderingData.cameraData.camera.GetComponent<connectSuntoFullVolumeNebulaURP>();
           // currentCamera = Camera.main;// renderingData.cameraData.camera;
            ...
            if (connector == null)
            {
                connector = renderingData.cameraData.camera.gameObject.GetComponent<connectSuntoFullVolumeNebulaURP>();
            ...
            if (enableFog && connectorFound)
            {
                ...
                {
                    //cmd.GetTemporaryRT(Shader.PropertyToID(m_TemporaryColorTexture.name), opaqueDesc, filterMode);//v0.1 //v0.1a
                    ...
                    {
                        RenderFullVolumetricClouds(renderingData.cameraData.camera, cmd, opaqueDesc);
                        //if (sunShaftIntensity != 0)
                        //{
                        //    RenderShafts(context, renderingData, cmd, opaqueDesc);
                        //}
                    }
        ...
        //v0.1 RG
        public void setNebulaVariables(connectSuntoFullVolumeNebulaURP connector)
        {
                ...
                //v0.8a
                maxPillars = connector.maxPillars;
                pillarTransforms = connector.pillarTransforms;
                pillarFaderA = connector.pillarFaderA;
                pillarFaderB = connector.pillarFaderB;
                pillarLimiterA = connector.pillarLimiterA;
                pillarType = connector.pillarType;
                //v0.8
                //v2.0
                ...
                debugNoLowFreqNoise = connector.enableVORTEX;// debugNoLowFreqNoise;
                ...
                if(connector.randomJitterNoise == connectSuntoFullVolumeNebulaURP.RandomJitterChoice.BlueNoise)
                {
                    randomJitterNoise = RandomJitter.BlueNoise;
                }
                if (connector.randomJitterNoise == connectSuntoFullVolumeNebulaURP.RandomJitterChoice.Off)
                {
                    randomJitterNoise = RandomJitter.Off;
                }
                if (connector.randomJitterNoise == connectSuntoFullVolumeNebulaURP.RandomJitterChoice.Random)
                {
                ...
                this.sunTransform = connector.sunLight.transform.position;// new Vector3(connector.Sun.x, connector.Sun.y, connector.Sun.z);// connector.sun.transform.position;
        ...
        {           
            ...
            Camera camera = Camera.main;// currentCamera;// Camera.main;
...
#else
            cmd.GetTemporaryRT(lrDepthBuffer.id, opaqueDesc, filterMode);//v0.1
#endif
...
#else
                cmd.Blit(source, lrDepthBuffer.id, sheetSHAFTS, 3);
#endif
...
#else
                cmd.Blit(source, lrDepthBuffer.id, sheetSHAFTS, 2);
#endif
...
#else
            cmd.Blit(source, m_TemporaryColorTexture.id); //KEEP BACKGROUND
#endif
...
#else
                cmd.Blit(lrDepthBuffer.id, lrColorB, sheetSHAFTS, 1);
                cmd.ReleaseTemporaryRT(lrDepthBuffer.id);//v0.1
#endif
 ...
 cmd.GetTemporaryRT(Shader.PropertyToID(lrDepthBuffer.name), opaqueDesc, filterMode); //v0.1               
                ...
                cmd.GetTemporaryRT(lrDepthBuffer.id, opaqueDesc, filterMode); //v0.1               
                //v0.1               
                cmd.Blit(lrColorB, lrDepthBuffer.id, sheetSHAFTS, 1);
#endif
            ...
            //v0.1         
#if UNITY_2022_1_OR_NEWER
            cmd.Blit( m_TemporaryColorTexture, source, sheetSHAFTS, (screenBlendMode == BlitFullVolumeNebulaSRP.BlitSettings.ShaftsScreenBlendMode.Screen) ? 0 : 4);
            cmd.ReleaseTemporaryRT(Shader.PropertyToID(lrDepthBuffer.name));//v0.1
            cmd.ReleaseTemporaryRT(Shader.PropertyToID(m_TemporaryColorTexture.name));//v0.1 
#else
            cmd.Blit(m_TemporaryColorTexture.id, source, sheetSHAFTS, (screenBlendMode == BlitFullVolumeNebulaSRP.BlitSettings.ShaftsScreenBlendMode.Screen) ? 0 : 4);
            cmd.ReleaseTemporaryRT(lrDepthBuffer.id);//v0.1
            cmd.ReleaseTemporaryRT(m_TemporaryColorTexture.id);//v0.1 
#endif
       ...
       // public int resolution = 256;
        ...
        public void RenderFullVolumetricClouds(Camera cameraA, CommandBuffer cmd, RenderTextureDescriptor opaqueDesc)
        //public override void Render(PostProcessRenderContext context)
        {
            ...
            Camera CurrentCamera = cameraA;// currentCamera;
            ...
            //v0.8a
            if (CloudMaterial.HasProperty("_maxPillars") && pillarTransforms.Count > 0) //check for other shader versions
            {
                int countLights = pillarTransforms.Count;
                if (countLights > maxPillars)
                {
                    countLights = maxPillars;
                }
                Quaternion globalRot = Quaternion.identity;
                CloudMaterial.SetInt("_maxPillars", maxPillars);
                Vector4[] posArray = new Vector4[countLights];
                Matrix4x4[] rotArray = new Matrix4x4[countLights];
                Vector4[] scaleArray = new Vector4[countLights];
                for (int i = 0; i < maxPillars; i++)
                {                    
                    posArray[i] = pillarTransforms[i].position;
                   // rotArray[i].SetTRS(pillarTransforms[i].position, pillarTransforms[i].rotation * globalRot, Vector3.one);
                    rotArray[i].SetTRS(pillarTransforms[i].position, Quaternion.Inverse(pillarTransforms[i].rotation) * globalRot, Vector3.one);
                    scaleArray[i] = new Vector4(pillarTransforms[i].localScale.x, pillarTransforms[i].localScale.y, pillarTransforms[i].localScale.z,pillarType[i]);
                }
                CloudMaterial.SetVectorArray("_PillarsArrayPos", posArray);
                CloudMaterial.SetMatrixArray("_PillarsArrayRot", rotArray);
                CloudMaterial.SetVectorArray("_PillarsArrayScale", scaleArray);
                CloudMaterial.SetFloat("pillarFaderA", pillarFaderA);
                CloudMaterial.SetFloat("pillarFaderB", pillarFaderB);
                CloudMaterial.SetFloat("pillarLimiterA", pillarLimiterA);//
            }
            else
            {
                CloudMaterial.SetInt("_maxPillars", 0);
            }
            //v0.8
            //v1.9.9.1
            // Debug.Log(_material.HasProperty("lightsArrayLength"));
            //Debug.Log(_material.HasProperty("controlByColor"));
            if (CloudMaterial.HasProperty("lightsArrayLength") && lightsArray.Count > 0) //check for other shader versions
            {
                //pass array
                CloudMaterial.SetVectorArray("_LightsArrayPos", new Vector4[maxImpostorLights]);
                CloudMaterial.SetVectorArray("_LightsArrayDir", new Vector4[maxImpostorLights]);
                int countLights = lightsArray.Count;
                if (countLights > maxImpostorLights)
                {
                    countLights = maxImpostorLights;
                }
                CloudMaterial.SetInt("lightsArrayLength", countLights);
                ...
                CloudMaterial.SetVectorArray("_LightsArrayPos", posArray);
                CloudMaterial.SetVectorArray("_LightsArrayDir", dirArray);
                CloudMaterial.SetVectorArray("_LightsArrayColor", colArray);
                //material.SetFloatArray(array);
            }
            else
            {
                CloudMaterial.SetInt("lightsArrayLength", 0);
            }
            ...
            //Debug.Log(rtClouds.name);
            //Debug.Log(CloudMaterial.name);
            //Debug.Log(destination.name);            

            cmd.Blit( Shader.PropertyToID(destination.name), rtClouds, CloudMaterial, 10); //v0.1
            ...
            //RenderTexture tmpBuffer4 = RenderTexture.GetTemporary((int)(rtW / ((float)downSample)), (int)(rtH / ((float)downSample)), 0, format);
            ...
            if (enabledTemporalAA && Time.fixedTime > 0.05f)
            {
                ...
                Matrix4x4 mt = lastFrameViewProjectionMatrix * CurrentCamera.cameraToWorldMatrix;
                CloudMaterial.SetMatrix("_FrameMatrix", mt);

                Unity2022 = true;
                //cmd.Blit(rtClouds, tmpBuffer4);//v0.5b
                ...
                if (Unity2022){
                    ...
                    //v0.2
                    Blitter.BlitCameraTexture(cmd, m_CameraColorTarget, _handleA, CloudMaterial, 12);
                    //cmd.Blit(m_CameraColorTarget, tmpBuffer3);

                    cmd.Blit(_handleA, previousFrameTexture);
                    cmd.Blit(_handleA, rtClouds);

                    //v0.2
                    Blitter.BlitCameraTexture(cmd, m_CameraColorTarget, _handleA, CloudMaterial, 13);
            ...
            //CloudMaterial.SetTexture("_CloudTexP", tmpBuffer4);
            ...
            {
                //v0.1               
                cmd.Blit( tmpBuffer1, source, CloudMaterial, 11);
                RenderShaftsNEBULA(ref cmd, opaqueDesc);
            }
            else
            {
                //v0.1               
           ...
           // CommandBufferPool.Release(cmd);
            ...
            //v0.6
            //RenderTexture.ReleaseTemporary(tmpBuffer4);
        ...
        //SHAFTS_NEBULA
        public void RenderShaftsNEBULA(ref CommandBuffer cmd, RenderTextureDescriptor opaqueDesc)
        {
            opaqueDesc.depthBufferBits = 0;
            Material sheetSHAFTS = blitMaterial;
...
#if UNITY_2022_1_OR_NEWER
            //Debug.Log(lrDepthBuffer.nameID);//v0.1
            if (lrDepthBuffer == null)// || lrDepthBuffer.rt.width != rtW || lrDepthBuffer.rt.height != rtH)
            {
                //Debug.Log("Alloc");
                lrDepthBuffer = RTHandles.Alloc(rtW, rtH, colorFormat: GraphicsFormat.R32G32B32A32_SFloat, dimension: TextureDimension.Tex2D, name: "lrDepthBuffer");
            }
            //Debug.Log(lrDepthBuffer.rt.name);
            //cmd.GetTemporaryRT(Shader.PropertyToID(lrDepthBuffer.name), opaqueDesc, filterMode);//v0.1
#else
            cmd.GetTemporaryRT(lrDepthBuffer.id, opaqueDesc, filterMode);//v0.1
#endif
...
#if UNITY_2022_1_OR_NEWER
                 cmd.Blit( source, lrDepthBuffer, sheetSHAFTS, 3); //v0.1  
#else
                cmd.Blit(source, lrDepthBuffer.id, sheetSHAFTS, 3);
#endif
...
#if UNITY_2022_1_OR_NEWER
                cmd.Blit( lrDepthBuffer, lrColorB, sheetSHAFTS, 1);
                //cmd.ReleaseTemporaryRT(Shader.PropertyToID(lrDepthBuffer.name));//v0.1 //v0.1a
#else
                cmd.Blit(lrDepthBuffer.id, lrColorB, sheetSHAFTS, 1);
                cmd.ReleaseTemporaryRT(lrDepthBuffer.id);//v0.1
#endif
...
#if UNITY_2022_1_OR_NEWER
              //  cmd.GetTemporaryRT(Shader.PropertyToID(lrDepthBuffer.name), opaqueDesc, filterMode);//v0.1 //v0.1a
                //v0.1               
                cmd.Blit( lrColorB, lrDepthBuffer, sheetSHAFTS, 1);
#else
                cmd.GetTemporaryRT(lrDepthBuffer.id, opaqueDesc, filterMode);//v0.1
            ...
            {
                m_TemporaryColorTexture = RTHandles.Alloc(rtW, rtH, colorFormat: GraphicsFormat.R32G32B32A32_SFloat, dimension: TextureDimension.Tex2D, name: "_TemporaryColorTexture");
               // m_TemporaryColorTexture = RTHandles.Alloc("_TemporaryColorTexture", name: "_TemporaryColorTexture");
            }
            //Debug.Log(m_TemporaryColorTexture.rt.name);

            //v0.1               
            cmd.Blit( m_TemporaryColorTexture, source, sheetSHAFTS, (screenBlendMode == BlitFullVolumeNebulaSRP.BlitSettings.ShaftsScreenBlendMode.Screen) ? 0 : 4);
            //return;
            //cmd.ReleaseTemporaryRT(Shader.PropertyToID(lrDepthBuffer.name));//v0.1//v0.1a
            //cmd.ReleaseTemporaryRT(Shader.PropertyToID(m_TemporaryColorTexture.name));//v0.1 //v0.1a
#else
            cmd.SetGlobalTexture("_ColorBuffer", lrDepthBuffer.id);
            //v0.1               
            cmd.Blit(m_TemporaryColorTexture.id, source, sheetSHAFTS, (screenBlendMode == BlitFullVolumeNebulaSRP.BlitSettings.ShaftsScreenBlendMode.Screen) ? 0 : 4);
                    ...
                    if (tmpBuffer == null)
                    {
						...
						if (blendBackground) { //v2.1.20
                            ...
                            if (isForReflections)
                            {
                                ...
                                //v0.1               
                		cmd.Blit(backgroundCam.targetTexture, tmpBuffer); //URP
                            }
                            else
                            {
                                ...
                                //v0.1               
                		cmd.Blit( backgroundCam.targetTexture, tmpBuffer, backgroundMat); //URP
                            }
						}
                        ...
                        else
                        {
                            ...
                            { //v2.1.20
                                ...
                                if (isForReflections)
                                {
                                    ...
                                    //Graphics.Blit(backgroundCam.targetTexture, tmpBuffer);
                                    //v0.1               
                			cmd.Blit( backgroundCam.targetTexture, tmpBuffer);
                                }
                                else
                                {
                                    backgroundCam.Render();
                                    ...
                                    //Graphics.Blit(backgroundCam.targetTexture, tmpBuffer, backgroundMat);
                                    //v0.1               
                			cmd.Blit( backgroundCam.targetTexture, tmpBuffer, backgroundMat);
                                }
                            }
        ...
        void renderMe(Camera cam, Material _material, RenderTextureDescriptor opaqueDesc, ScriptableRenderContext context, CommandBuffer cmd)
        {
            ...
            //v0.1               
            cmd.Blit( tmpBuffer1, source, _material, 6);
...
#if UNITY_2022_1_OR_NEWER
        void CustomGraphicsBlitOpt(ScriptableRenderContext context, RenderTargetIdentifier source, RTHandle dest, RenderTexture skysource, Material fxMaterial, int passNr, Gradient DistGradient, //dest, RenderTexture skysource, Material fxMaterial, int passNr, Gradient DistGradient,
        ...
        {
#else
        void CustomGraphicsBlitOpt(ScriptableRenderContext context, RenderTargetIdentifier source, RenderTargetHandle dest, RenderTexture skysource, Material fxMaterial, int passNr, Gradient DistGradient, //dest, RenderTexture skysource, Material fxMaterial, int passNr, Gradient DistGradient,
                   Vector2 GradientBounds, Texture2D colourPalette, Camera cam, bool toggle, bool splitPerFrame, bool WebGL, CommandBuffer cmd, RenderTextureDescriptor opaqueDesc)
        {
#endif
            //URP
            ...
            if (toggle)
            {
                ...
                //v0.1               
                cmd.Blit(source,new1,fxMaterial,6);
                ...
                //v0.1               
                cmd.Blit(new1, _cloudBuffer); //URP
                //release
              ...
              //  RenderTexture.active = tmpBuffer2;
                ...
                if (splitPerFrame && cameraMotionCompensate)
                {
                  //  fxMaterial.SetPass(8); //4
                    //v0.1               
                cmd.Blit( source, tmpBuffer2, fxMaterial, 8);
                }
                else
                {
                    //fxMaterial.SetPass(7);//3
                    //v0.1               
                cmd.Blit( source, tmpBuffer2, fxMaterial, 7);
                }
