Shader "ORION/FullVolumeNebulaURP"
{
	Properties
	{
		//// CLOUDS
...
//		}
	...
	float4 raymarch(float3 rayOrigin, float3 ro, float3 rd, float steps, float depth, float cosAngle, float2 duv)
	{
		float3 pos = ro;
		float4 res = 0.0; // cloud color
		float lod = 0.0;
...
//#endif
		...
		float4 clouds3D = raymarch(ro,rs, rd * stepSize, steps, depth, cosAngle, duv);// fogFac; // raymarch volumetric clouds
