Shader "ORION/FullVolumeNebulaURP_PILLARS"
{
	Properties
	{
		//// CLOUDS
...
#include "ClassicNoise3D.hlsl"
	...
	#define BIG_STEP 3.0
			...
			//v0.8a
			int _maxPillars = 0;
			float4 _PillarsArrayPos[32];
			float4x4 _PillarsArrayRot[32];
			float4 _PillarsArrayScale[32];
			float pillarFaderA;
			float pillarFaderB;
			float pillarLimiterA;
...
//		}
	...
	float4 raymarch(float3 rayOrigin, float3 ro, float3 rd, float steps, float depth, float cosAngle, float2 duv, float2 texcoord)
	{
		float3 pos = ro;
		float4 res = 0.0; // cloud color
		float lod = 0.0;
		...
		for (int i = 0; i < lightsArrayLength; i++)
		{
			...
			float dist2 = length(lightPos - _CameraWS);

			float2 pos2d = WorldToScreenPos(lightPos);
			float uvsDist = 1 * length(pos2d - texcoord) / 1;
			uvsDist = 1 - uvsDist.rrr;

			float outOfViewCheck = 0;
			if (dot(normalize(forward), normalize(lightPos - _CameraWS)) >= 0) {
				outOfViewCheck = 1;
			}
			//depth test
			float lightPower2 = lightPower;
			if (length(lightPos - _CameraWS) > length(pos - _CameraWS)) { //if behind obstacle, zero intensity
				lightPower2 = 0;
			}
			...
			//addLight = addLight * (1 / dist2);

			//addLight = addLight * 1 + 1*(
			//	lightColor * addLight * (1 / pow(uvsDist, (1 / lightFalloff * 3.2))) * lightPower2 * 0.0009 / (dist2 * 0.1 + 0.15),-10.2,11.9) * outOfViewCheck *1;

			//addLight = lightColor* lightPower * (1 / pow(uvsDist, (1 / lightFalloff * 3.2)));// addLight * 1 + (
					///lightColor * addLight * (1 / pow(uvsDist, (1 / lightFalloff * 3.2))) * lightPower2 * 0.0009 / (dist2 * 0.1 + 0.15)) * outOfViewCheck * o;

			//addLight = addLight + 0.001*saturate(11111111*lightColor * (1 / pow(uvsDist, (1 / (lightFalloff * 3.2)))));
					//lightColor * addLight * (1 / pow(uvsDist, (1 / lightFalloff * 3.2))) * lightPower2 * 0.0009 / (dist2 * 0.1 + 0.15)) * outOfViewCheck * o;

			//addLight = addLight * 1 + (
			//	lightColor * addLight * (1 / pow(uvsDist, (1 / lightFalloff * 3.2))) * lightPower2 * 0.0009 / (dist2 * 0.1 + 0.15)) * outOfViewCheck * o;

			//addLight = 1001110*saturate(1111111*float3(11, 0, 0) * (1 / pow(uvsDist, (25))));

			//addLight = addLight * 1 + (
			//	lightColor * 1 * (1 / pow(uvsDist, (1 / (lightFalloff/10000) * 3.2))) * lightPower2 * 0.0009 / (dist2 * 0.1 + 0.15)) * outOfViewCheck *1;

			/*return float4(pow(1-uvsDist.rrr, lightFalloff / 10) * lightColor.rgb* lightPower*0.0001, 0.6);
			return float4(_WorldSpaceCameraPos.rgb * 0.004, 1);
			return float4(_CameraWS.rgb * 0.004, 1);
			return float4(dist2.rrr*0.0004, 1);
			return float4(dir.rgb, 1);
			return float4(lightColor.rgb, 1);
			return float4(lightFalloff.rrr * 0.0001, 1);
			return float4(lightFalloff.rrr*0.1, 1);
			return float4(lightPower.rrr, 1);
			return float4(lightDir.rgb, 1);
			return float4(lightPos.rgb, 1);
			return float4(lightPower2.xxx, 1);
			return float4(lightPower.xxx, 1);
			return float4(addLight.xyz, 0);*/

			//addLight = addLight * 1 + (
			//	lightColor * 1 * (1 / pow(uvsDist.rrr, (1 / (lightFalloff / 1) * 1))) * lightPower2 * 0.0009 / (dist2 * 0.1 + 0.15)) * outOfViewCheck * lightPower;

			addLight = addLight + saturate(float3(pow( uvsDist, lightFalloff / 1) * lightColor.rgb * lightPower * 0.001));
		}
		//addLight = float3(1, 0, 0);
		
		//return float4(_LightsArrayColor[2].xyz,1);// saturate(float4(addLight, 1)) * 211;
		...
		for (float i = 0.0; i < steps; i += stepLength)
		{
			...
			//float3 test = 0;
			//float addPillar = 0;
			for (int i = 0; i < _maxPillars; i++)
			{
				float3 pillarPos = _PillarsArrayPos[i].xyz + 0;// _WorldSpaceCameraPos;// +_CameraWS;
				float4x4 pillarRot = _PillarsArrayRot[i];
				float3 pillarScale = _PillarsArrayScale[i].xyz;
				int pillarType = _PillarsArrayScale[i].w;				

				if (pillarType == 0)
				{
					test = mul(pillarRot, pos - pillarPos);
					
					float3 dist = abs(test) - 0.5 * pillarScale;
					//dist = pow(dist, 2);
					float distA = (min(max(dist.x, max(dist.y, dist.z)), 0.0) + length(max(dist, 0.0)));
					//distA = pow(distA,0.25)*10;
					addPillar = max(addPillar, 1 - (distA + pillarLimiterA));
				}

				//test = addPillar;

				//addPillar += 0.1;
				//float pillarFalloff = pillarFaderA;
				//float3 pillarColor = _LightsArrayColor[i].xyz;
			}
			//addPillar = pow(addPillar, 0.16);
			addPillar = saturate(((addPillar * pillarFaderA - 0.5f) * max(1, 0.7)) + 0.25f);
			//addPillar = pow(addPillar, 2);
			...
			float cloudControl = 1;
			if (addPillar == 0) {
				//stepLength = zeroCount > 10.0 ? BIG_STEP : 1.0; // check if we need to do big or small steps
				//pos += rd * stepLength;				
				//continue;
				cloudControl = 0;
				//return 0;// float4(test, 1);
			}

			if (distance(_CameraWS, pos) >= depth || res.a >= 0.999) { // check if is behind some geometrical object or that cloud color aplha is almost 1
																	  //break;  // if it is then raymarch ends

																	  //v0.1 - add option to rernder in front of all objects for reflections
				
				//NEBULA
				c = c * (1-res.a) * 1;// c * (1 - res.a*res.a*res.a*res.a);
				//starFieldASum = starFieldASum * (1 - res.a);

				//v0.8a
				c = (c - 10 * (c * addPillar * addPillar * addPillar * addPillar * addPillar * 0.00025));
				c = c * cloudControl;
...
//#endif
			...
			//v0.8a
			//cloudDensity = cloudDensity + cloudDensity* addPillar * addPillar * addPillar *0.000025;
			//cloudDensity = cloudDensity - saturate(cloudDensity * addPillar * addPillar * addPillar * 0.000025);
			cloudDensity = (cloudDensity - 100*(cloudDensity * addPillar * addPillar * addPillar * addPillar * addPillar * 0.000025));

			cloudDensity = cloudDensity * cloudControl;

			////NEBULA
			//float SIZE = 0.22;
			//float den = 0.045; float tileNum = 1;
			//float3 q = frac(p*tileNum) - 0.5;
			//float3 id = floor(p*tileNum);
			//float2 rn = Hash33(id).xy;
			...
			//float size = (Hash13(id)*0.2 + 0.8)*SIZE;
			//float demp = pow(1. - size / SIZE, .8)*0.45;
			//float val = (sin(1 * 31.*size)*demp + 1. - demp) * size * 1;
			//float c2 = 1 - smoothstep(0., val, length(q));
			//c2 *= step(rn.x, (.0005 + i * i*0.001)*den);
			//if (res.a < 0.5 && length(p) < 211000) {
			//	c += c2 * (lerp(float3(1.0, 0.49, 0.1), float3(0.75, 0.9, 1.), rn.y)*0.25 + 0.75) * (1 - res.a)*0.2;
			//}
			//p *= 1.15;// 1.4;
			...
			{			
				...
				if (sunRaysOn == 3) {
					...
					//v0.8a
					//particle.rgb = float4(1, 0, 0, 1);_SunColor
					particle.rgb = 5*(particle.rgb * 1 - 211111 * (particle.rgb * addPillar * addPillar * addPillar * addPillar * addPillar * 0.000025))* _SunColor;
					particle.rgb =  particle.rgb * (1- length(addLight)) + addLight* 1;// particle.rgb + particle.rgb * addLight * 1;
...
//#endif
		...
		float4 clouds3D = raymarch(ro,rs, rd * stepSize, steps, depth, cosAngle, duv, i.uv);// fogFac; // raymarch volumetric clouds
		//return clouds2D + clouds3D * 3	;


		//NEBULA v0.1
		//clouds3D.rgb += raymarchSTARS(ro, rd * stepSize, steps, depth, cosAngle, duv);
...
#pragma fragment FragGrey
				...
				Pass // 12
			{
				...
				float4 frag(VaryingsA  input) : SV_Target
				{
					...
					float prevDepth = SAMPLE_DEPTH_TEXTURE(_PreviousDepth, sampler_PreviousDepth, float2(reprojCoord.xy *0.5+0.5 + oneOverResolution * 0.0)).x;
				...
				float4 previousWorldPosition = mul(_LastFrameInverseViewProjectionMatrix, float4(reprojCoord.xy *0.5+0.5, prevDepth, 1.0));
				...
				//float4 prevGI = SAMPLE_TEXTURE2D(_PreviousColor, sampler_PreviousColor, float2(reprojCoord.xy *0.5+0.5));
