Shader "VFX/Shuriken Cloud"
{
	Properties
	{
		...
		// Cloud
		_CloudAttenuationRamp("_CloudAttenuationRamp", 2D) = "white" {}
		_CloudBackScattering("_CloudBackScattering", Float) = 1.0
		_FakeLightDirection("_FakeLightDirection", Vector) = (0.2,-0.6,0.2,1.0)
		_FakeLightColor("_FakeLightColor", Color) = (1.0,1.0,1.0,1.0)
		_FakeLightIntensity("_FakeLightIntensity", Float) = 1.0
	...
	SubShader
	{
		...
		Pass
		{
			...
			// Debug : for renderdoc (disable if not needed)
			#pragma enable_d3d11_debug_symbols

			// SKN Master Features	
			#pragma shader_feature COLORSOURCE_RGBA COLORSOURCE_ALPHA COLORSOURCE_ALTERNATEALPHA
			#pragma shader_feature _ VFX_LIGHTING VFX_LIGHTING_FAKEDIRECTIONAL VFX_LIGHTING_RAYMARCH2D
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				col.rgb *= _Brightness;
				col.rgb *= SKM_GET_CLOUD_LIGHTING(col.a);

				//#ifdef VFX_LIGHTING_RAYMARCH2D
				col.a = 1.0 - (1.0 / pow(4.0 * col.a + 1.0, 4.0));
				//#endif
	...
	CustomEditor "Artngame.SKYMASTER.UnityEditor.ShurikenCloudGUI"	
}
