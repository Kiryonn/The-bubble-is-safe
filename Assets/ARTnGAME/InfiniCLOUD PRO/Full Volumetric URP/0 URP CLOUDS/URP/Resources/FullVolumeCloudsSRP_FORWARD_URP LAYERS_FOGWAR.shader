Shader "Unlit/FullVolumeCloudsURP LAYERS Fog of War"
{
	Properties
	{
		//// CLOUDS
		...
		_SunPosition("sun pos", Color) = (111, 11,339, 11)
			...
			//v0.9
			superCellControlsA("super Cell Controls A", Float) = (0, 0, 0, 0)
			...
			//v0.9
			fogOfWarColor("fogOfWarColor", Color) = (0.0, 0.0, 0.0, 0.0)
	}
...
//#endif		
	...
	float3 cloudsColorFluid = float3(0, 0, 0); //v1.5
	...
	float3 sampleWeather(float3 pos) {
		...
		if (_maskScale.x > 0) {
			...
			float3 maskData = tex2Dlod(_maskTexture, float4((pos.xz) * 0.00000125* _maskScale.xy + float2(0.5, 0.5), 0, 0)).rgb;
			weatherData.r = weatherData.r * (1 - maskData.r);
			//cloudsColorFluid = saturate(float3(pow(maskData.r, 3), 0, 0))*fogOfWarRadius;// *(1 + cos(25 * _TimeA.x)); //v1.5
			cloudsColorFluid = saturate(float3(pow(maskData.r, 3), pow(maskData.g, 3), pow(maskData.b, 3)) * fogOfWarColor.rgb )*fogOfWarRadius;// *(1 + cos(25 * _TimeA.x)); //v1.5 - v0.9
		}
		else {
			//v24d
			weatherData.r = weatherData.r * (1 - 0.2);
		}
	...
	float getHeightFractionForPoint(float3 pos)
	{
		...
		return give;//saturate((distance(pos, _PlanetCenter) - (_SphereSize + _CloudHeightMinMax.x)) / _Thickness);
#endif
	}
...
//#endif
		...
		return calculateLightEnergy(densityAlongCone, cosAngle, density) * _SunColor + cloudsColorFluid; //v1.5
	}
...
//		}
	...
	float4 raymarch(float3 ro, float3 rd, float steps, float3 depth, float cosAngle, float2 duv)
	{
		float3 pos = ro;
		float4 res = 0.0; // cloud color
		float lod = 0.0;
		...
		{
			if (distance(_CameraWS, pos) >= length(depth)
...
//#endif
			...
			else if(sunRaysPower != 0)// if cloud sample was 0, then increase zero cloud sample counter //v1.3
			{			
				...
				if (sunRaysOn == 1) {
					...
					res = sunRaysPower *(1.0 - res.a) * 0.0008 * 1 * 1 * directLight.b * float4(1.5, 0.7, 0.5, 1) * 11 + res;//v1.3 sunRaysPower
	...
	float4 frag_FV(v2f_FV i) : SV_Target
	{
		...
		//v0.9
		depth *= _FarPlane * cameraScale;// *YCutHeightDepthScale.y;
...
//#if defined(ALLOW_IN_CLOUDS) // if in cloud flying is allowed, then figure out if camera is below, above or in the cloud layer and set 
		...
		if (distanceCameraPlanet < _SphereSize + _CloudHeightMinMax.x - _DistanceParams.y) // Below clouds, v0.1 subtract 21000
		{
			...
			&& (superCellControlsA.x == 0 || distanceCameraPlanet < _SphereSize + _CloudHeightMinMax.y + superCellControlsA.x)) // Above clouds //v1.5
		{
			//float give = distance(pos, _PlanetCenter) - (_SphereSize + _CloudHeightMinMax.x); //v1.2 Beta 24
			//if (superCellControlsA.w != 0 && give > 1000 * superCellControlsA.z* (weatherData.r)) {
			//	give += superCellControlsA.x;
...
//#endif
		...
		float4 clouds3D = raymarch(rs, rd * stepSize, steps, wsDir, cosAngle, duv); //v0.9
...
//
//		//superCellControlsA
//		//_CloudHeightMinMax.x = superCellControlsA.x;
//		_Thickness = superCellControlsA.y;
//		//_CloudHeightMinMax.y = _CloudHeightMinMax.x + _Thickness;
//
...
//#endif
			float dotSun = dot(v3LightDir, normalize(rs - _PlanetCenter));// _SunDir); //v0.9b
			if (dotSun < checkVar)
			{
				clouds3D.rgb = 0;
			}
...
//#endif
	...
	inline half LuminanceA(half3 rgb)
	{
	...
	half4 frag_nodepth(v2f i) : SV_Target{
		...
		UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(i);
			...
			if (LuminanceA(abs(sky.rgb - tex.rgb)) < 0.2) {
				outColor = TransformColor(tex) * dist;
...
//		XY.x = XY.x / 2;
	...
	float2 WorldToScreenPos(float3 pos) {
		pos = normalize(pos - _WorldSpaceCameraPosC)*(_ProjectionParams.y + (_ProjectionParams.z - _ProjectionParams.y)) + _WorldSpaceCameraPosC;
	...
	//	float depth =  Linear01Depth(zsample * (zsample < 1.0)); //////// CHANGE 2 URP
		...
		if (noise3D == 0) {
			...
			float hor1 = -_cameraDiff.w * signer1 *_cameraDiff.y * 2210 / 1 * (dividerScale / 1) - 1.2 * _WorldSpaceCameraPosC.x * 10 + fixFactor1;
			float hor2 = -_cameraDiff.w * signer2 *_cameraDiff.y * 2210 / 1 * (dividerScale / 1) - 1.2 * _WorldSpaceCameraPosC.x * 10 + fixFactor2;
			float hor3 = -_cameraDiff.w * signer2 *_cameraDiff.y * 1210 / 1 * (dividerScale / 1) - 1.2 * _WorldSpaceCameraPosC.x * 2 + fixFactor2;

			float vert1 = _cameraTiltSign * _cameraDiff.x * 0.77 * 1.05 * 160 + 0.0157*_cameraDiff.y * (pow((input.texcoord.x - 0.1), 2)) - 0.3 * _WorldSpaceCameraPosC.y * 30
				- 2 * 0.33 * _WorldSpaceCameraPosC.z * 2.13 + 50 * abs(cos(_WorldSpaceCameraPosC.z * 0.01)) + 35 * abs(sin(_WorldSpaceCameraPosC.z * 0.005));

			float vert2 = _cameraTiltSign * _cameraDiff.x * 0.20 * 1.05 * 160 + 0.0157*_cameraDiff.y * (pow((input.texcoord.x - 0.1), 2)) - 0.3 * _WorldSpaceCameraPosC.y * 30
				- 1 * 0.33 * _WorldSpaceCameraPosC.z * 3.24 + 75 * abs(sin(_WorldSpaceCameraPosC.z * 0.02)) + 85 * abs(cos(_WorldSpaceCameraPosC.z * 0.01));

			float vert3 = _cameraTiltSign * _cameraDiff.x * 0.10 * 1.05 * 70 + 0.0117*_cameraDiff.y * (pow((input.texcoord.x - 0.1), 2)) - 0.3 * _WorldSpaceCameraPosC.y * 30
				- 1 * 1.03 * _WorldSpaceCameraPosC.z * 3.24 + 75 * abs(sin(_WorldSpaceCameraPosC.z * 0.02)) + 85 * abs(cos(_WorldSpaceCameraPosC.z * 0.01));
				...
				//#endif
			{
				...
				float3 pointToCamera = (wpos - _WorldSpaceCameraPosC) * 0.47;
				int steps = 2;
				float stepCount = 1;
				float step = length(pointToCamera) / steps;
				for (int j = 0; j < steps; j++) {
					//ray trace noise												
					float3 coordAlongRay = _WorldSpaceCameraPosC + normalize(pointToCamera) * step
		...
		if (1 == 1) {
			...
			if (
				length(_WorldSpaceCameraPosC - wpos) < length(_WorldSpaceCameraPosC - light1) - lightRadius
				&&
				dot(normalize(_WorldSpaceCameraPosC - wpos), normalize(_WorldSpaceCameraPosC - light1)) > 0.95// 0.999
				) { //occlusion
			}
			else {
				float factorOcclusionDist = length(_WorldSpaceCameraPosC - wpos) - (length(_WorldSpaceCameraPosC - light1) - lightRadius);
				float factorOcclusionDot = dot(normalize(_WorldSpaceCameraPosC - wpos), normalize(_WorldSpaceCameraPosC - light1));
	...
	{
		//float4 back = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv);
		//float4 back = SAMPLE_TEXTURE2D(_CloudTexP, sampler_CloudTexP, float2(i.uv.x, i.uv.y*controlCloudEdgeOffset)); //*0.9968));//v0.6
		...
		//float4 cloudA = SAMPLE_TEXTURE2D(_CloudTexP, sampler_CloudTexP, i.uv);	
		//cloud.a = cloudA.a;
		//return cloud*(1) + 2*back*(cloud.a);
		//return cloud;
		//return back;//cloud*(cloud.a)*back + 18*back*(1-cloud.a);
		//return float4(1,0,0,1);
		...
		{
			if (depth < 0.001)// && depth > ) //BACKGROUND
			{
				cloud.a = 1;
				float depthOffsetA = 0.00001 * depthDilation;
				float4 cloudA1 = SAMPLE_TEXTURE2D(_CloudTex, sampler_CloudTex, i.uv + float2(depthOffsetA, depthOffsetA));
				float4 cloudA2 = SAMPLE_TEXTURE2D(_CloudTex, sampler_CloudTex, i.uv + float2(-depthOffsetA, depthOffsetA));
				float4 cloudA3 = SAMPLE_TEXTURE2D(_CloudTex, sampler_CloudTex, i.uv + float2(depthOffsetA, -depthOffsetA));
				float4 cloudA4 = SAMPLE_TEXTURE2D(_CloudTex, sampler_CloudTex, i.uv + float2(-depthOffsetA, -depthOffsetA));
				cloud.rgb += (cloudA1 + cloudA2 + cloudA3 + cloudA4) / 4;
				//return 0;
			}
			//return 0;
		}
		//return cloud.a;
		if (depthPixel < 0.2) {
			return  float4(back.rgb, 1);
		}
		return  float4(depth * back.rgb * (1 - cloud.a), 1) + float4(cloud.rgb * 1, 1);
		////
...
#pragma fragment FragGrey
				...
				Pass // 12
			{
				...
				//float temporalMultiplier = 0.1*exp(-300 * length(pos.xy - temporalUV));//NEW TAA
				//gi = lerp(prevGI, gi, saturate(float4(blendWeight, blendWeight, blendWeight, blendWeight)*0.65 * _TemporalGain / temporalMultiplier));
			...
			Pass //14 BLIT
			{
				...
				#include "BlitSkyMaster.hlsl"//v0.2

				#pragma vertex Vert
				#pragma fragment FragNearest
			ENDHLSL
			}
			Pass //15 BLIT BACKGROUND
			{
				Name "ColorBlitPasss"
				HLSLPROGRAM
				#include "Packages/com.unity.render-pipelines.universal/ShaderLibrary/Core.hlsl"
		...
		ENDHLSL
			...
			//// END GRAPH
		Pass{ // 16 - Redo Blend with Blitter
				ZTest Always Cull Off ZWrite Off
				HLSLPROGRAM

				#include "BlitSkyMaster.hlsl"//v0.2
				#pragma vertex Vert
				#pragma fragment fragCombineCloudsA

				half4 fragCombineCloudsA(v2f i) : SV_Target{
					float4 back = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, float2(i.uv.x, i.uv.y * controlCloudEdgeOffset)); //*0.9968));//v0.6
					float4 cloud = SAMPLE_TEXTURE2D(_CloudTex, sampler_CloudTex, i.uv);
					//float4 cloudA = SAMPLE_TEXTURE2D(_CloudTexP, sampler_CloudTexP, i.uv);	
					//cloud.a = cloudA.a;
					//return cloud*(1) + 2*back*(cloud.a);
					//return cloud + back* (1-cloud.a);
					//return back;//cloud*(cloud.a)*back + 18*back*(1-cloud.a);
					//return float4(1,0,0,1);
					...
					return  float4(depth * back.rgb * (1 - cloud.a), 1) + float4(cloud.rgb *1, 1);
					//			return (controlCloudEdgeA.x*float4(back.rgb* (controlCloudEdgeA.y - 0), 1)
					//				+ controlCloudEdgeA.z*float4(cloud.rgb,1))//* pow(cloud.a, controlCloudAlphaPower + 0.001), 1))
					//				*controlCloudEdgeA.w *depth + float4((1 - depth) * back.rgb, 1);
