...
{
    [ExecuteInEditMode]
    public class connectSuntoFullVolumeCloudsURP : MonoBehaviour
    {
        [Tooltip("Unity 2022 new Blitter, requires PATCH inside the Resources folder with the Clouds shader")]
        public bool Unity2022 = false; //requires PATCH inside the Resources folder with the Clouds shader
        ...
        //v0.8 - Mask texture and Fog of War
        ...
        //v0.4a
        public bool renderAfterTransparent = false;
        public float heightBetweenCloudAndWater = 250;
        public bool autoAdjustTranspOrder = false; //if below middle height defined above, render clouds after transparent (make above bool true), if above put false
        ...
        public float cameraScale = 1;// 80; //use 80 for correct cloud scaling in relation to land //v5.3.0
        public float extendFarPlaneAboveClouds = 1;

        public int cloudChoice = 1; //v5.3.0 was 0
        ...
        public bool allowFlyingInClouds = true;//v5.3.0 was false
        [Range(1, 8)]
        public float downSample = 2;//v5.3.0 was 1
        ...
        public float planetSize = 1200000;// 35000.0f; //v5.3.0
        public Vector3 planetZeroCoordinate = new Vector3(0.0f, 0.0f, 0.0f);
        //[Range(0.0f, 1.0f)]
        public float scale = 0.2f;//0.3f;  //v5.3.0
        public float noiseCuttoff = 0.2f; //v5.3.0 was 1
        [Range(0.0f, 32.0f)]
        public float detailScale = 1;//13.9f; //v5.3.0
        [Range(0.0f, 1.0f)]
        public float lowFreqMin = 0.5f;// 0.366f;//v5.3.0
        [Range(0.0f, 1.0f)]
        public float lowFreqMax = 0.7f;//0.8f;//v5.3.0
        [Range(0.0f, 1.0f)]
        public float highFreqModifier = 0.4f;//0.21f; //v5.3.0
        [Range(0.0f, 10.0f)]
        public float curlDistortScale = 7.44f;
        [Range(0.0f, 1000.0f)]
        public float curlDistortAmount = 407.0f;
        [Range(0.0f, 1.0f)]
        public float weatherScale = 0.056f;// 0.1f; //v5.3.0
        //[Range(0.0f, 2.0f)]
        public float coverage = 1.4f; // 0.92f; //v5.3.0
        [Range(0.0f, 2.0f)]
        public float cloudSampleMultiplier = 1.5f;// 1.0f; //v5.3.0
        ...
        [Range(0.0f, 1.0f)]
        public float highCloudsScale = 0.025f;//v5.3.0 was 0.5
        ...
        [Range(0.0f, 5f)]//1.5
        public float sunLightFactor = 2.1f;// 0.79f; //v5.3.0
        ...
        public float globalMultiplier = 15.0f; //v5.3.0
        ...
        public float localLightPosScaling = 1; //v0.2 - scale position to match cloud rendering in distance, use 8 for full volume clouds lightning
        public float currentLocalLightIntensity = 0.1f;//v5.4.2
        ...
        public Vector4 _InteractTextureAtr = new Vector4(1, 1, 1, 1); //new Vector4(1,0.5f,1,1); //v5.3.0 was empty 0,0,0,0
        ...
        public float luminance = 0.85f; //v5.3.0 was 1
        ...
        public float mieDirectionalG = 0.9f; //v5.3.0 was 1
        public float bias = 5;//v5.3.0  was 1
        public float contrast = 10;//v5.3.0 was 1
        ...
        void Update()
        {
            ...
            if (sunLight == null && !Application.isPlaying)
            {
                ...
                controlWeatherTOD_SM_URP weathercontroller = transform.gameObject.GetComponent<controlWeatherTOD_SM_URP>();
            ...
            //v0.6
            //put controlBackAlphaPower = 1, controlCloudAlphaPower = 0.001 if inside or above clouds
            if (autoRegulateEdgeMode && Application.isPlaying && Camera.main != null)
            { //the new edges mode is only for view below clouds, regulate out of the mode if inside or above
                if (Camera.main.transform.position.y > autoRegulateCutoffDistance)//stop new edge blend mode when near or above clouds
                {
                    ...
                    depthDilation = 0;
                    //edgeControl = new Vector4(1, 1, 1, 1);
            ...
            //v0.4a
            if (autoAdjustTranspOrder && Camera.main != null)
            {
                if (Camera.main.transform.position.y > heightBetweenCloudAndWater)
                {
                    renderAfterTransparent = true;
                }
                else
                {
                    renderAfterTransparent = false;
                }
            }
