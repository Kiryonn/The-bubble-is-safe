...
Shader "SkyMaster/ShaderVolumeClouds-Desktop-SM3.0 v3.4.4 VORTEX ScriptLess H" {
    Properties {
		...
		[HDR]
		_LightColor0A ("_LightColor0", COLOR) = (1,1,1,1) 
    }
    SubShader {
        Tags {
            ...
            float4 frag(VertexOutput i) : COLOR {
               ...
               // float4 paintTexture1 = tex2D(_PaintMap,UVs1+_CloudMap1_ST);
            	...
            	float3 Difference = i.worldPos.xyz - _WorldSpaceCameraPos;Difference.y=worldPos_y - _WorldSpaceCameraPos.y;//v0.1
					...
					float cosTheta = dot(normalize(Difference), sunPosition);// (float3(sunPosition.x, sunPosition.y, sunPosition.z)) ); //v3.3					
					...
					float diffuseFactor = max(dot(normalN.xyz, sunPosition),0);  // float3(sunPosition.x, sunPosition.y, sunPosition.z) ), 0);  //v3.3
					float3 lightDirection = sunPosition.xyz;         				
					float3 lightColor = _LightColor0A.rgb;
...
//             	}
      ...
      //       	float shaper = (_Transparency-0.48+0.0)*((DER1*saturate((_Coverage+cloudTexture1.a)))-Texture2.a);		////////////////////////////////////////
                ...
                float3 lightDirect = normalize(sunPosition.xyz);
...
//               float3 finalCol = diff*_LightColor0.rgb* atten;	
                 float3 finalCol = diff*_SunColor.rgb* 1;//atten ;
...
//                float4 Fcolor = float4(saturate(endColor*0.9 + 0.1*endColor*_FogFactor*1*diff2*_FogColor*0.000000001*1),saturate(shaper - 0.01*(_HorizonFactor*0.00001*diff2*1)  )) ;
				...
				Fcolor.rgb = Fcolor.rgb * (pow(_LightColor0A,0.2 + sunPosition.w*0.1))+ float3(1, 1, 1) * localLightPower;
...
//             	}
 ...
 //              	float3 endColor = _LightColor0.rgb*lerp(2.5*(atten+0.015)*_LightColor0.rgb*_ShadowColor.rgb*(1-(Texture1.g)),   11*Texture1.g*0.05*atten*_LightColor0.rgb*ColB,    Lerpfactor);
				...
				if(_WorldSpaceLightPos0.w == 0.0){
               	...
               	float3 endColor =diff*_LightColor0A.rgb*lerp(_ShadowColor.rgb,  ColB,    (1-Lerpfactor)) *atten;
