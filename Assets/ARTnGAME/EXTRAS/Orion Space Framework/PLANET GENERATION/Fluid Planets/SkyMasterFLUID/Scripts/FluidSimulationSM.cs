...
{
    public class FluidSimulationSM
    {
        //v0.1
       ...
       // [SerializeField]
        ...
        public void SimStep(float dt)
        {
            ...
            ComputeShader.SetVector("powerControlsA", new Vector4(dragFlowPower, ripplePower, patternPower, outwardsPower));
            ComputeShader.SetVector("powerControlsB", new Vector4(cosinePower, blastPower, 1, 1));

            //v0.1
            // Add external force
            ComputeShader.SetVector("ForceOrigin", input);
            ComputeShader.SetFloat("ForceExponent", _exponent);
            ComputeShader.SetTexture(Kernels.Force, "V4Field_out", V_IN);
            ComputeShader.SetTexture(Kernels.Force, "V4Field_in", V_OUT);
            bool mouse1pressed = false;
            bool mouse2pressed = false;
            if (Input.GetMouseButton(1))
            {
                // Random push
                ComputeShader.SetVector("ForceVector", Random.insideUnitCircle * _force * 0.025f);
                mouse2pressed = true;
            }
            else if (Input.GetMouseButton(0))
            {
                // Mouse drag
                ComputeShader.SetVector("ForceVector", (input - _previousInput) * _force);
               // ComputeShader.SetVector("ForceVector", Random.insideUnitCircle * _force * 0.025f);
            ...
            {
                ComputeShader.SetVector("ForceVector", Vector4.zero);
            }
            ComputeShader.Dispatch(Kernels.Force, ThreadCountX, ThreadCountY, 1);


            // Add Velocity
            if (1==1 && Input.GetKey(KeyCode.P) || mouse1pressed)
            {
            ...
            // Add Dye
            if (Input.GetKey(KeyCode.L) || mouse1pressed)
            {
            ...
            // Run Simulation
            if (Input.GetKey(KeyCode.O) || mouse1pressed || mouse2pressed || constantUpdate)
            {
                // Debug.Log("Key O");
