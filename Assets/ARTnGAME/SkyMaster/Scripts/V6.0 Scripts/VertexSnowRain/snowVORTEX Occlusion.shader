...
Shader "SkyMaster/snowRainVERTEX_SM_VORTEX Occlusion" {
    Properties {
		...
		tilingOffsetDepth("tilingOffsetDepth", Vector) = (1,1,0,0)
		oneMinus("oneMinus (1,1 for one minus or 0,-1)", Vector) = (1,1,0,0)
    }
	SubShader {
        ...
        Pass {
			...
			//v0.8
			uniform sampler2D _topDownDepth;
			float2 oneMinus;
            ...
            v2f vert(appdata_custom v)
            {
				...
				//}					
					...
					//v0.8
					float3 worldPos = mul (unity_ObjectToWorld, finalposition).xyz;
					o.wpos = worldPos;
            ...
            {	
				...
				//float4 texD = tex2D(_topDownDepth, float2(1-(i.wpos.x * tilingOffsetDepth.x)+ tilingOffsetDepth.z, 1-(i.wpos.z * tilingOffsetDepth.y) + tilingOffsetDepth.w));
				//float4 texD = tex2D(_topDownDepth, float2((i.wpos.x * tilingOffsetDepth.x)+ tilingOffsetDepth.z, (i.wpos.z * tilingOffsetDepth.y) + tilingOffsetDepth.w));

				float4 texD = tex2D(_topDownDepth, float2(
				1*oneMinus.x-oneMinus.y*(i.wpos.x * tilingOffsetDepth.x+ tilingOffsetDepth.z), 
				1*oneMinus.x-oneMinus.y*(i.wpos.z * tilingOffsetDepth.y + tilingOffsetDepth.w)
				));

				if(texD.r < 0.05){
					//return float4(tex.rgb + float3(0,2,2), pow(tex.a,alphaPow) * alpha)*2;
					return float4(tex.rgb, pow(tex.a,alphaPow) * alpha)*5;
				}else{
					discard;
				}
				return 0;
				if(i.wpos.z < 0 && i.wpos.x < 0){
					//discard;
					return float4(tex.rgb + float3(211,0,0), pow(tex.a,alphaPow) * alpha)*1;
				}
				//discard;
