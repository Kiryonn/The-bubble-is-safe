...
namespace Artngame.SKYMASTER
{
    ...
    public class biomesSkyMaster : MonoBehaviour
    {
        ...
        //v0.5
        public bool advancedCloudShaping = false;        
        public List<float> biomeRaysResX = new List<float>();
        public List<float> biomeRayShadowW = new List<float>();
        public List<float> biomeStartHeight = new List<float>();
        public List<float> biomeThickness = new List<float>();
        public List<float> biomePlanetSize = new List<float>();
        public List<float> biomeCloudNoiseScale = new List<float>();
        public List<float> biomeCloudNoiseCutoff = new List<float>();
        public List<float> biomeLowFreqMin = new List<float>();
        public List<float> biomeLowFreqMax = new List<float>();
        public List<float> biomeHighFreqMod = new List<float>();
        public List<float> biomeWeatherScale = new List<float>();
        public List<float> biomeExtendFarPlane = new List<float>();

        //v0.7 - cloud shaping
        public bool planetShaping = false;
        public List<Vector4> secondCloudLayer = new List<Vector4>();
        ...
        //v0.6
        public bool controlLightning = false;
        public List<float> biomeLightningPower = new List<float>();
        public List<float> biomeLightningRange = new List<float>();
        public List<float> biomeLightningRate = new List<float>();
        public List<Color> biomeLightningColor = new List<Color>();
#if URP
        public lightningCameraVolumeCloudsSM_URP lightning;
#else
            ...
            if (Player != null)
            {
                for (int i = 0; i < biomesCenters.Count; i++)
                {
                    ...
                    )
                    {
                        ...
                        {
#if URP
                            if (i < biomesFogColor.Count) {//Beta 24d
                        ...
                        //v0.5
                        if (advancedCloudShaping)
                        {
                            if (!useMultiTypeClouds || (useMultiTypeClouds && biomeCloudType[currentBiomeID] == 0)) //CLOUD TYPE 0 = FULLY VOLUMETRIC with FLY THROUGH
                            {
                                fullVolumeClouds.raysResolution.x = Mathf.Lerp(fullVolumeClouds.raysResolution.x, biomeRaysResX[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.rayShadowing.w = Mathf.Lerp(fullVolumeClouds.rayShadowing.w, biomeRayShadowW[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.startHeight = Mathf.Lerp(fullVolumeClouds.startHeight, biomeStartHeight[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.thickness = Mathf.Lerp(fullVolumeClouds.thickness, biomeThickness[i], Time.deltaTime * cloudLerpSpeed);

                                fullVolumeClouds.planetSize = Mathf.Lerp(fullVolumeClouds.planetSize, biomePlanetSize[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.noiseScale = Mathf.Lerp(fullVolumeClouds.noiseScale, biomeCloudNoiseScale[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.noiseCuttoff = Mathf.Lerp(fullVolumeClouds.noiseCuttoff, biomeCloudNoiseCutoff[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.lowFreqMin = Mathf.Lerp(fullVolumeClouds.lowFreqMin, biomeLowFreqMin[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.lowFreqMax = Mathf.Lerp(fullVolumeClouds.lowFreqMax, biomeLowFreqMax[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.highFreqModifier = Mathf.Lerp(fullVolumeClouds.highFreqModifier, biomeHighFreqMod[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.weatherScale = Mathf.Lerp(fullVolumeClouds.weatherScale, biomeWeatherScale[i], Time.deltaTime * cloudLerpSpeed);
                                fullVolumeClouds.extendFarPlaneAboveClouds = Mathf.Lerp(fullVolumeClouds.extendFarPlaneAboveClouds, biomeExtendFarPlane[i], Time.deltaTime * cloudLerpSpeed);
                            }
                        }
                        ...
                        if (planetShaping)
                        {
                            if (!useMultiTypeClouds || (useMultiTypeClouds && biomeCloudType[currentBiomeID] == 0)) //CLOUD TYPE 0 = FULLY VOLUMETRIC with FLY THROUGH
                            {
                                fullVolumeClouds.superCellControls = Vector4.Lerp(fullVolumeClouds.superCellControls, secondCloudLayer[i], Time.deltaTime * cloudLerpSpeed);                                
                            }
                        }
                        ...
                        if (controlLightning)
                        {
                            ...
                            if (lightning != null)
                            {
                                lightning.lightning_every = Mathf.Lerp(lightning.lightning_every, biomeLightningRate[i], Time.deltaTime * cloudLerpSpeed);
                                if (!useMultiTypeClouds || (useMultiTypeClouds && biomeCloudType[currentBiomeID] == 0)) //CLOUD TYPE 0 = FULLY VOLUMETRIC with FLY THROUGH
                                {
                                    //FULL VOLUME CLOUDS
                                    fullVolumeClouds.localLightFalloff = Mathf.Lerp(fullVolumeClouds.localLightFalloff, biomeLightningRange[i], Time.deltaTime * cloudLerpSpeed);
                                    fullVolumeClouds.localLightIntensity = Mathf.Lerp(fullVolumeClouds.localLightIntensity, biomeLightningPower[i], Time.deltaTime * cloudLerpSpeed);
                                    //fullVolumeClouds.localLightRadius
                                    if (fullVolumeClouds.localLight != null)
                                    {
                                        fullVolumeClouds.localLight.color = Color.Lerp(fullVolumeClouds.localLight.color, biomeLightningColor[i], Time.deltaTime * cloudLerpSpeed *2);
                                    }
                                }
                                else
                                {
                                    //BACKGROUND CLOUDS
                                    volumeClouds.localLightFalloff = Mathf.Lerp(volumeClouds.localLightFalloff, biomeLightningRange[i], Time.deltaTime * cloudLerpSpeed);
                                    volumeClouds.localLightIntensity = Mathf.Lerp(volumeClouds.localLightIntensity, biomeLightningPower[i], Time.deltaTime * cloudLerpSpeed);
                                    //volumeClouds.localLightRadius
                                    if (fullVolumeClouds.localLight != null)
                                    {
                                        volumeClouds.localLight.color = Color.Lerp(volumeClouds.localLight.color, biomeLightningColor[i], Time.deltaTime * cloudLerpSpeed * 2);
                                    }
                                }
                            }
#else
